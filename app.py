    ...
    if not all_faces_lms:
        ...
        for idx, lms in enumerate(all_faces_lms, start=1):
            ...
            with st.spinner(f"Mengekstrak fitur Wajah #{idx}..."):
                ...
                lbp_ycbcr = extract_lbp_ycbcr(face_crop)              # 2368 fitur (Y + CbCr)
                gabor = extract_gabor(face_crop)                      # 32 fitur

            # Persiapan Vektor Fitur Sesuai Dimensi Model
            feat_antro = antro.reshape(1, -1)
            feat_antro13 = antro13.reshape(1, -1)
            feat_pure_lbp = pure_lbp.reshape(1, -1)
            feat_a = np.concatenate([lbp_ycbcr, antro]).reshape(1, -1)
            ...
            with col1:
                scaled = artifacts["antro_standalone"]["scaler"].transform(feat_antro)
                show_prediction_column("Antropometri", artifacts["antro_standalone"]["model"], scaled)

            with col2:
                scaled = artifacts["antro_frontal"]["scaler"].transform(feat_antro)
                show_prediction_column("Antro Frontal", artifacts["antro_frontal"]["model"], scaled)

            with col3:
                scaled = artifacts["antro13_frontal"]["scaler"].transform(feat_antro13)
            ...
            with col5:
                scaled = artifacts["lbp_ycbcr_antro"]["scaler"].transform(feat_a)
                show_prediction_column("LBP + Antro", artifacts["lbp_ycbcr_antro"]["model"], scaled)
